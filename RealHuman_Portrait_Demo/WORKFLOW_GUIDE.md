...
2. **Advanced Creation Rights** — Required to access the Real-Human Portrait Library.
...
> **Warning:** To enable `Strategy: "Skip"`, you must first **turn off Secure Mode** in the console. This operation is **irreversible** — once disabled, Secure Mode cannot be re-enabled, and console asset management is permanently disabled.
...
- Index is **per type**: images and audio are numbered independently
- **Never** reference an Asset ID directly in the prompt text
- Asset URI format: `asset://<asset_id>` in `content[n].image_url.url` or `content[n].audio_url.url`

### `role` field placement

`role` is a **sibling of `type`** in the content item, **not** nested inside `image_url`/`audio_url`:
