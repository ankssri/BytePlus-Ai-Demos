...
def search_news_articles(query, limit=3, language=None, category=None):
    ...
    #if language == 'hindi':
     #   query = re.sub(r'[़-ॿ]', lambda x: x.group().normalize('NFKD'), query)  # Normalize diacritics
    try:
        ...
        # Scenario 2: User typed query (semantic search)
            ...
            search_params = {
                "text": query,
                "limit": 50,
                "dense_weight": 0.7,  # Increased from default 0.5 <mcreference link="https://docs.byteplus.com/en/docs/VikingDB/pysdk_search_with_multi_modal#request-parameters" index="0">0</mcreference>
                "filter": {
                    "op": "must",
                    "field": "language",
                    "conds": [language]
...
with st.sidebar:
    ...
    st.image("https://ankurdemo.tos-ap-southeast-1.bytepluses.com/newsArticle/VikingDB.svg", use_container_width=False,width=150)
    st.markdown(''' :white[BytePlus VikingDB Vector Database] ''')
    st.image("https://ankurdemo.tos-ap-southeast-1.bytepluses.com/newsArticle/tos.svg", caption="Byteplus Object Storage", use_container_width=False,width=150)
    # Add credits or additional information
    st.markdown("### Data Source")
    st.write("Citation paper : News articles from various Indian publications in multiple languages. https://huggingface.co/datasets/ai4bharat/IndicHeadlineGeneration")
